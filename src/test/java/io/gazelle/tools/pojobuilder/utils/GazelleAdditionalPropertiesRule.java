...
import io.gazelle.utils.GazellePodamStrategy;
 ...
 */
public class GazelleAdditionalPropertiesRule implements Rule<JDefinedClass, JDefinedClass> {

	private final RuleFactory ruleFactory;

	protected GazelleAdditionalPropertiesRule(RuleFactory ruleFactory) {
	...
	public JDefinedClass apply(String nodeName, JsonNode node, JDefinedClass jclass, Schema schema) {
		...
		if (!ruleFactory.getAnnotator().isAdditionalPropertiesSupported()) {
			// schema allows additional properties, but serializer library can't
			// support them
			return jclass;
		}
	...
	private JFieldVar addAdditionalPropertiesField(JDefinedClass jclass, JType propertyType) {
		...
		field.annotate(PodamCollection.class).param("nbrElements", 2)
				.param("mapElementStrategy", GazellePodamStrategy.class);
	...
	private void addSetter(JDefinedClass jclass, JType propertyType, JFieldVar field) {
		...
		JMethod setterAll = jclass.method(JMod.PUBLIC, void.class, "setAdditionalProperties");
		JVar setterAllParam = setterAll.param(field.type(), "additionalProperties");
		setterAll.body().assign(JExpr._this().ref(field), setterAllParam);
