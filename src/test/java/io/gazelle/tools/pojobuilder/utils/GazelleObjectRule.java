 ...
 */
public class GazelleObjectRule implements Rule<JPackage, JType> {

	private final RuleFactory ruleFactory;

	protected GazelleObjectRule(RuleFactory ruleFactory) {
	...
	private JDefinedClass createClass(String nodeName, JsonNode node, JPackage _package)
		...
		try {
			...
			if (node.has("javaTypeName")) {
				nodeName = node.get("javaTypeName").asText();
			}
	...
	private void addPojomaticAnnotation(JDefinedClass jclass) {
		jclass.annotate(AutoProperty.class);
	}
	...
	private void addToString(JDefinedClass jclass) {
		...
		Class<?> toStringBuilder = Pojomatic.class;

		JBlock body = toString.body();
		JInvocation reflectionToString = jclass.owner().ref(toStringBuilder).staticInvoke("toString");
	...
	private void addHashCode(JDefinedClass jclass) {
		...
		Class<?> hashCodeBuilder = Pojomatic.class;

		JBlock body = hashCode.body();
		JInvocation hashCodeBuilderInvocation = jclass.owner().ref(hashCodeBuilder).staticInvoke("hashCode");
		hashCodeBuilderInvocation.arg(JExpr._this());
	...
	private void addConstructors(JDefinedClass jclass, List<String> properties) {
		...
		// no properties to put in the constructor => default constructor is
		// good enough.
	...
	private void addEquals(JDefinedClass jclass) {
		JMethod equalMethod = jclass.method(JMod.PUBLIC, boolean.class, "equals");
		JVar otherObject = equalMethod.param(Object.class, "o");

		Class<?> equalsCodeBuilder = Pojomatic.class;

		JBlock body = equalMethod.body();
		JInvocation hashCodeBuilderInvocation = jclass.owner().ref(equalsCodeBuilder).staticInvoke("equals");
		hashCodeBuilderInvocation.arg(JExpr._this());
		hashCodeBuilderInvocation.arg(otherObject);

		body._return(hashCodeBuilderInvocation);

		equalMethod.annotate(Override.class);
	}
