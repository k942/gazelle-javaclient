...
public class GazellePropertyRule implements Rule<JDefinedClass, JDefinedClass> {

	private final RuleFactory ruleFactory;

	protected GazellePropertyRule(RuleFactory ruleFactory) {
	...
	public JDefinedClass apply(String nodeName, JsonNode node, JDefinedClass jclass, Schema schema) {
		...
		if (node.has("title")) {
			...
			// ruleFactory.getTitleRule().apply(nodeName, node.get("title"),
			// setter, schema);
		}
		...
		if (node.has("description")) {
			...
			// ruleFactory.getDescriptionRule().apply(nodeName,
			// node.get("description"), setter, schema);
		}
		...
		if (node.has("required")) {
			...
			// ruleFactory.getRequiredRule().apply(nodeName,
			// node.get("required"), setter, schema);
		}
